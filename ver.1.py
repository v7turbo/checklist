...
path = pathlib.Path.home()                                           # Путь к домашней директории
path_file = Path(path, 'Desktop', 'test', 'base.xls')                # Путь к таблице для всех ОС
excel_data = pd.read_excel(path_file, usecols=column, skiprows=row)  # Чтение определенного диапазона
